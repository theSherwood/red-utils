...
suite "block series, with index, 1 for, no if" [
	test "series as word" [
		...
		assert equal? series/only [
			reduce [a-elem a-idx] for a-elem a-idx in a
		] [[5 1] [6 2] [7 3]]
		unset 'a
	]
	test "series inlined" [
...
suite "string series, no index, 1 for, no if" [
	...
	test "series inlined" [
		...
		assert equal? series/into/only [
			reduce [a-elem] for a-elem in "abc"
		] [] [[#"a"] [#"b"] [#"c"]]
	]
]
...
suite "string series, with index, 1 for, no if" [
	...
	test "series inlined" [
		...
		assert equal? series/into [
			rejoin [a-elem a-idx] for a-elem a-idx in "abc"
		] [] ["a1" "b2" "c3"]
		assert equal? series/into [
			reduce [a-elem a-idx] for a-elem a-idx in "abc"
		] [] [#"a" 1 #"b" 2 #"c" 3]
		assert equal? series/into/only [
			reduce [a-elem a-idx] for a-elem a-idx in "abc"
		] [] [[#"a" 1] [#"b" 2] [#"c" 3]]
...
suite "if clauses" [
	...
	test "block, index, 1 for" [
		...
		assert equal? series [
			a-elem * a-idx for a-elem a-idx in a if even? a-elem + a-idx
		] [5 12 21 32]
		unset 'a
	]
]
...
suite "2 for" [
	test "block" [
		...
		b: [1 -1 2]
		assert equal? series [
			a-elem * b-elem for a-elem in a for b-elem in b
	...
	test "block, if clause" [
		...
		b: [1 -1 2]
		assert equal? series [
			a-elem * b-elem for a-elem in a for b-elem in b if even? a-elem + b-elem
		] [5 -5 12 7 -7]
		unset [a b]
	]
	test "mixed series, if clause" [
		...
		b: "abc"
		assert equal? series [
			rejoin [a-elem b-elem] for a-elem in a for b-elem in b if odd? a-elem
		] ["5a" "5b" "5c" "7a" "7b" "7c"]
		assert equal? series [
			rejoin [a-elem b-elem] for b-elem in b for a-elem in a if odd? a-elem
		] "5a7a5b7b5c7c"
		assert equal? series/into [
			rejoin [a-elem b-elem] for b-elem in b for a-elem in a if odd? a-elem
		] [] ["5a" "7a" "5b" "7b" "5c" "7c"]
